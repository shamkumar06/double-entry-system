...
export default function HomeScreen({ onSelectProject }) {
    ...
    const [newProject, setNewProject] = useState({ name: '', description: '', totalFunds: '', logoUrl: '', initialReceived: '' });
    const [saving, setSaving] = useState(false);
    const [uploadingLogo, setUploadingLogo] = useState(false);
    const [editingId, setEditingId] = useState(null);
    const [editData, setEditData] = useState({ name: '', description: '', totalFunds: '', logoUrl: '' });
    ...
    const handleCreate = async (e) => {
        ...
        setSaving(true);
        try {
            const project = await accountingApi.createProject({
                name: newProject.name.trim(),
                description: newProject.description.trim(),
                totalFunds: parseFloat(newProject.totalFunds) || 0,
                logoUrl: newProject.logoUrl
            });

            // ── Auto-record Initial Funding if provided ──────────────────
            const initialAmt = parseFloat(newProject.initialReceived);
            if (initialAmt > 0) {
                try {
                    const categories = await accountingApi.listCategories();
                    // Find suitable accounts (Asset for Debit, Equity for Credit)
                    const assetAcc = categories.find(c => c.type === 'ASSET') || categories[0];
                    const equityAcc = categories.find(c => c.type === 'EQUITY' || c.type === 'REVENUE') || categories[0];

                    if (assetAcc && equityAcc) {
                        await accountingApi.createTransaction({
                            projectId: project.id,
                            date: new Date().toISOString().split('T')[0],
                            description: 'Initial Project Funding',
                            lines: [
                                { accountId: assetAcc.id, type: 'DEBIT', amount: initialAmt },
                                { accountId: equityAcc.id, type: 'CREDIT', amount: initialAmt }
                            ]
                        });
                    }
                } catch (txErr) {
                    console.error("Failed to create initial funding transaction", txErr);
                    // We don't fail project creation if the initial tx fails, but we notify
                    alert("Project created, but failed to record initial funding transaction.");
                }
            }

            setNewProject({ name: '', description: '', totalFunds: '', logoUrl: '', initialReceived: '' });
            setCreating(false);
            await fetchProjects();
        } catch (e) {
    ...
    const handleLogoUpload = async (e) => {
        ...
        try {
            ...
            setNewProject(prev => ({ ...prev, logoUrl: url }));
        } catch (err) {
            console.error(err);
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            await accountingApi.updateProject(editingId, {
                ...
                totalFunds: parseFloat(editData.totalFunds) || 0,
                                    ...
                                    <div key={`edit-${project.id}`} className="glass-panel" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', border: '1px solid var(--primary)' }}>
                                        <form onSubmit={handleUpdate} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                            ...
                                            <div>
                                                ...
                                                <input type="number" step="0.01" value={editData.totalFunds} onChange={e => setEditData({...editData, totalFunds: e.target.value})} placeholder="0.00" style={{ padding: '0.5rem', marginTop: '0.2rem', width: '100%', borderRadius: '6px', border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text-main)' }} />
                                            </div>
                                            <div>
                                ...
                                <div key={project.id} className="glass-panel" style={{ padding: '1.75rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', cursor: 'pointer', transition: 'transform 0.2s ease, box-shadow 0.2s ease' }}
                                    ...
                                    <div style={{ marginTop: '0.5rem' }}>
                                        ...
                                        <p style={{ color: 'var(--secondary)', fontWeight: 600, fontSize: '0.95rem', textAlign: 'center' }}>
                                            Fund: {formatCurrency(project.totalFunds || 0)}
                                        </p>
                                    </div>
                ...
                {creating && (
                    <div className="glass-panel" style={{ padding: '2rem', marginTop: '1.5rem' }}>
                        ...
                        <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                                <div>
                                    <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)', fontSize: '0.875rem' }}>Total Funds Allocation ({currency})</label>
                                    <input type="number" step="0.01" value={newProject.totalFunds} onChange={e => setNewProject({...newProject, totalFunds: e.target.value})} placeholder="Project Budget" />
                                </div>
                                <div>
                                    <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--accent)', fontSize: '0.875rem', fontWeight: 600 }}>Initial Funds Received ({currency})</label>
                                    <input type="number" step="0.01" value={newProject.initialReceived} onChange={e => setNewProject({...newProject, initialReceived: e.target.value})} placeholder="Actual Handover" 
